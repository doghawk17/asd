...
function checkForInterviewConflicts($pdo, $gig_id, $time_start, $time_end) {
	$sql = "SELECT * FROM gig_interviews 
			WHERE gig_id = ? AND 
			((time_start BETWEEN ? AND ?) OR 
			(time_end BETWEEN ? AND ?) OR 
			(time_start <= ? AND time_end >= ?))";
	$stmt = $pdo->prepare($sql);
	$stmt->execute([$gig_id, $time_start, $time_end, $time_start, $time_end, $time_start, $time_end]);
...
function insertNewGigInterview($pdo, $gig_id, $freelancer_id, $time_start, $time_end) {
	...
	if ($time_start < $current_datetime) {
		return false; // Cannot schedule interviews in the past
	}

	// Check for conflicts with other interviews
	if (checkForInterviewConflicts($pdo, $gig_id, $time_start, $time_end)) {
		return false; // Conflict with existing interview
	}
